...
def select_02():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT nombre, apellido, email FROM estudiantes WHERE fecha_nacimiento > '2001-12-31'")
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
...
def select_03():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT nombre, creditos FROM cursos ORDER BY creditos DESC")
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
...
def select_04():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("SELECT e.nombre, c.nombre, i.nota FROM estudiantes e JOIN inscripciones i ON e.id = i.estudiante_id JOIN cursos c ON i.curso_id = c.id")
    for r in cursor.fetchall():
        print(dict(r))
    conn.close()
...
def insert_04():
    ...
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO estudiantes (nombre, apellido, email, fecha_nacimiento) VALUES ('Valentina', 'Ruiz', 'valentina.ruiz@email.com', '2004-01-01'), ('Mateo', 'Torres', 'mateo.torres@email.com', '2003-05-20')")
    conn.commit()
